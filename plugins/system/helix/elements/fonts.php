	...
	protected function getInput() {
		...
		window.addEvent('domready', function(){
			...
			function getActiveFontList() {
				data_sources.each(function(el,j){el.removeClass('inactive').addClass('active');});
				if ($('{$this->id}_type').value=='google') {
					data_sources[1].removeClass('active').addClass('inactive');
				} else {
					data_sources[0].removeClass('active').addClass('inactive');
				}
